...
_FEATURE_KEYWORDS: tuple[str, ...] = (
    "feature:", "feat:", "feature", "feat", "new feature", "add support",
    "enable", "implement", "introduce", "enhance", "enhancement", "improve",
)
_BUG_KEYWORDS: tuple[str, ...] = (
    "bug:", "bug", "broken", "fail", "crash", "not work",
...
def build_compact_report(
    ...
    if not new_features:
        ...
        for entry in new_features:
            ...
            icon = "🆕" if item_type == "release" else "🔧"
            sections.append(f"- {icon} [{title}]({url})（{repo_label}）")
        sections.append("")
    ...
    if not known_issues:
        ...
        for entry in known_issues:
            ...
            url = item.get("html_url", "")
            repo_label = item.get("_repo", "")
            sections.append(f"- ❗ [{title}]({url})（{repo_label}）")
        sections.append("")
